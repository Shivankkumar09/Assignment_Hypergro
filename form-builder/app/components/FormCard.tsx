...
export default function FormCard({ field }: Props) {
...
/>
      ...
      {(field.type === "dropdown" || field.type === "checkbox" || field.type === "radio") && (
        <div className="mb-2">
          <div className="space-y-1">
            {(field.options || []).map((opt, idx) => (
              <div key={idx} className="flex justify-between items-center text-sm  bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">
                <span>{opt}</span>
                <button
          ...
          <div className="flex mt-2 gap-2">
            ...
            />
            <button onClick={handleAddOption} className="bg-blue-500 text-white px-3 py-1 rounded text-sm">
              Add
            </button>
          </div>
        </div>
      ...
      <div className="flex justify-between items-center mt-4 text-sm">
        <button onClick={() => dispatch(removeField(field.id))} className="text-red-600 hover:underline">
          Delete
        </button>
